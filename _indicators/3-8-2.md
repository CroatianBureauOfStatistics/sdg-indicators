---
layout: indikator
permalink: /3-8-2/
indicator_number: 3-8-2
reporting_status: complete
data_non_statistical: false
sdg_goal: 

goal_meta_link: http -//unstats.un.org/sdgs/files/metadata-compilation/Metadata-Goal-3.pdf
goal_meta_link_page: 34
graph: longitudinal
graph_negative:
graph_title: Udio stanovnika s velikim izdacima za zdravstvo,
većim od 10 % kao udio u ukupnim izdacima ili dohotku kućanstva (%)
has_metadata: false
indicator: 3.8.2
indicator_definition: "Za definiranje velikih izdataka kućanstva koriste se dva praga - veći od 10 % i veći od 25 % ukupnih izdataka ili dohotka kućanstva. Izvor - UN"
indicator_name: Udio stanovnika s velikim izdacima za zdravstvo kao udio u ukupnim izdacima ili dohotku kućanstva
indicator_variable: Udio stanovnika s velikim izdacima za zdravstvo,
većim od 10 % kao udio u ukupnim izdacima ili dohotku kućanstva (%)
layout: indicator
periodicity:
permalink: /3-8-2/
published: true
sdg_goal: 3
source_url: https -//www.dzs.hr/
target: Postići opću obuhvaćenost zdravstvenim osiguranjem, uključujući zaštitu od financijskih rizika, pristup kvalitetnim osnovnim zdravstvenim uslugama, te dostupnost sigurnih, učinkovitih, kvalitetnih i cijenom pristupačnih lijekova i cjepiva za sve.
target_id: 3.8
time_period:
title: Udio stanovnika s velikim izdacima za zdravstvo kao udio u ukupnim izdacima ili dohotku kućanstva
un_custodial_agency: |
  "WHO  (Partnering  Agencies -  World  Bank)
un_designated_tier: 1
indicator_variable_2:
indicator_variable_3:
---
